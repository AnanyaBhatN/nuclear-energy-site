    ...
    navigate: (pageName) => {
        ...
        setTimeout(() => {
            ...
            if (pages[pageName]) {
                ...
                app.innerHTML = pages['home']; // Default
            }
            ...
            // Update Nav State
            document.querySelectorAll('.nav-link').forEach(el => el.classList.remove('active', 'text-nuclear-600', 'dark:text-nuclear-400'));
            const activeBtn = document.getElementById(`nav-${pageName}`);
            if(activeBtn) {
                activeBtn.classList.add('active', 'text-nuclear-600', 'dark:text-nuclear-400');
            }
